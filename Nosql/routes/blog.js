...
router.get("/posts/:id", async function (re, res) {
...
const post = await db
    .getDb()
    .collection("posts")
    .findOne({ _id: new ObectId(postId) }, { summary: 0 });

  if (!post) {
    return res.status(404).render("404");
  }
  post.humanReadableDate = post.date.toLocalDateString("en-US", {
    weekday: "long",
    year: "numeric",
    month: "long",
    day: "numeric",
...
router.get('/posts/:id/edit', function(req, res){
  ...
  const post = await db
    .getDb()
    .collection("posts")
    .findOne({ _id: new ObectId(postId) }, { title: 1, summary: 1, body: 1 });
...
router.post('/posts/:id/edit', async function(req, res){
  const postId = new ObectId(req.params.id);
